...
include_once('test_load_fact.php');
  ...
  for ($row = 1; $row <= $highestRow; $row++) { 
    ...
    if ($rowData[0][1] <> '#N/A')
    {
//      $rowData[0][4] = PHPExcel_Style_NumberFormat::toFormattedString($rowData[0][4], 'YYYY-MM-DD');
//      $rowData[0][5] = PHPExcel_Style_NumberFormat::toFormattedString($rowData[0][5], 'YYYY-MM-DD');
...
/*      
      ...
      create_factura ($tipo_fact, $profesor, $clasificacion, $fecha, $fecha_contable, $grupo, $irpf_fac, $iva, $base);
      if ($ss <> 0)
      {
        echo '<pre>Creando SS</pre>';
        create_factura ('factura proveedor', $profesor, 'ss', $fecha, $fecha_contable, $grupo, 0, 0, $ss);
      }
      
      if ($irpf <> 0)
      {
        echo '<pre>Creando IRPF</pre>';
        create_factura ('factura proveedor', $profesor, 'irpf', $fecha, $fecha_contable, $grupo, 0, 0, $irpf);
      }
  */    
    echo '<pre>';
      print_r($rowData);
